...
function ProductDetailSection({
  ...
  const selectedProductImages = product.images.filter(
  ...
  const currentItemDetails = selectedSize
    ? product?.inventory.find(
        (inventoryItem) =>
          ...
          selectedColor === inventoryItem.color
      )
    : product?.inventory.find(
        (inventoryItem) => selectedColor === inventoryItem.color
      );

  const { list_price: mrp = 0, sale_price: discount_price = 0 } =
    currentItemDetails ?? {};
  ...
  const handleCartDispatch = () => {
    ...
    const temp = {
      ...
      ...currentItemDetails,
      color: currentItemDetails.color || "",
      size: currentItemDetails.size || "N/A",
      quantity,
      totalPrice,
    };
              ...
              image.color === selectedColor && (
                <div
                  ...
                  className={`object-cover h-[190px] w-[160px] rounded-lg overflow-hidden flex-shrink-0 ${
                    image.image_url ===
                    selectedProductImages[selectedImage]?.image_url
      ...
      <div className="flex-1 flex flex-col gap-8 overflow-hidden pl-[2px]">
        ...
        {product && (
          <SizeSelect
            ...
            onSizeButtonClick={handleSizeSelect}
          />
        )}
        <div className="flex flex-col gap-4">
          ...
          <div className="flex h-9 w-[125px] justify-around items-center border-[1px] border-neutral-200 bg-neutral-50 rounded-md text-neutral-600">
            <Button
              ...
              onClick={(e) => handleQuantityButtonClick(e)}
            >
              -
            </Button>
        ...
        <div className="">
          <Button
            className="w-full bg-indigo-700 hover:bg-indigo-800 font-medium text-lg py-3 text-white rounded-[4px] [box-shadow:0px_1px_2px_0_rgb(0_0_0_/_0.06),_0px_1px_3px_0_rgb(0_0_0_/_0.10)]"
